/* AGENZA on-site AI agent widget
   ---------------------------------------------------------------
   HOW IT WORKS:
   - The widget POSTs to "api/agent.php" on your hosting, which holds
     the OpenRouter key securely server-side (never in the browser).
   - If the API is unreachable (e.g. local file testing), the built-in
     smart brain answers instead — the widget never breaks.
*/
const AGENZA_AGENT = {
  ENDPOINT: "api/agent.php", // secure PHP proxy on your hosting (holds the key)
...
(function(){
  ...
  const T = {
    ar: {
      ...
      fallback: 'تمام، عشان أفيد حضرتك بدقة في النقطة دي يفضّل نكمل كلامنا واتساب أو تحجز ديمو مجاني، وده هياخد دقيقة واحدة. وفي نفس الوقت ممكن تسألني عن: الأسعار، الوكلاء، تأكيد الأوردرات، أو الشيتات.',
    ...
    en: {
      ...
      fallback: 'Noted — to advise you precisely on this, best we continue on WhatsApp or you book a free demo, it takes a minute. Meanwhile you can ask about: pricing, agents, order confirmation, or sheets.',
      waBtn: '💬 Continue on WhatsApp', demoBtn: '🎯 Book a free demo',
      err: 'Connection hiccup.. please resend your message 🙏',
    }
  }[LOC];

  // ---- smart brain (keyword → answer) ----
  const BRAIN = LOC === 'en' ? [
    ...
    { k:['مين','شركة','agenza','عنكم','بتعملوا'], a:'<b>AGENZA</b> شركة برمجيات مصرية متخصصة في وكلاء الذكاء الاصطناعي اللي بيردّوا ويأكّدوا ويسجّلوا — كأنهم بشر، بس مابيناموش. ⚡' },
  ...
  function withButtons(html, rule){
    let h = html;
    if(rule && rule.btn === 'wa') h += '<br><a class="ag-btn" target="_blank" href="' + (LOC==='en'?AGENZA_AGENT.WHATSAPP_EN:AGENZA_AGENT.WHATSAPP) + '">' + T.waBtn + '</a>';
    if(rule && rule.btn === 'demo') h += '<br><a class="ag-btn" href="' + (LOC==='en'?'index-en.html#contact':'index.html#contact') + '">' + T.demoBtn + '</a>';
    return h;
  }
  ...
  async function agentReply(msg){
    ...
    const rule = brain(msg); // used for helper buttons + offline fallback
    try{
      let html = null;
      if(AGENZA_AGENT.ENDPOINT){
        try{
          ...
          if(data && data.reply) html = esc(data.reply);
          else console.warn('[Amer] API error payload:', data);
        }catch(e){ console.warn('[Amer] API unreachable, brain fallback:', e && e.message); html = null; /* offline / local file → brain fallback below */ }
      }
      await wait; tp.remove();
      if(html){
        bubble(withButtons(html, rule), 'ai');
        showChips(rule && rule.c ? rule.c : null);
      } else {
        bubble(withButtons(rule ? rule.a : T.fallback, rule), 'ai');
        showChips(rule && rule.c ? rule.c : null);
      }
